...
Note that if $norm(dot)=norm(dot)^2$, then $norm(A)=sigma_1$ and $A^(-1)=1 / sigma_n$, where $sigma_1$ and $lambda_n$ are the maximum abnd minimum singular value of $A$. Thus
$ K(A) = sigma_1 / sigma_n $
...
#theorem("Finite-Termination Property")[
  For an $n times n$ SPD matrix $A$ , the CG algorithm converges to the exact solution $x = A^(-1)b$ in at most $n$ iterations in exact arithmetic.
]

#theorem("Error Bounds")[
  Let $A$ be an $n times n$ SPD matrix and let $x^((k))$ be the iterate of the CG method at the $k$-th step. Then, the error $e^((k))=x-x^((k))$ satisﬁes the following error bounds:
  $
    norm(e^((k)))_A lt.eq (2 c^k) / (1 + c^(2k)) norm(e^((0)))_A
  $
  where $c=(sqrt(K(A))-1) / (sqrt(K(A))+1)$ and $K(A)$ is the condition number of $A$.
]
